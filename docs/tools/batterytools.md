...
This document explains `docs/tools/battery_calculator.py`: a small hourly solar+battery simulator that uses a Peak Sun Hours (PSH) block model to estimate battery State‑of‑Charge (SoC) over multiple days.
...
```powershell
python docs/tools/battery_calculator.py
```
...
If you want, I can add a short header comment inside `battery_calculator.py` with the same explanation and example parameter sets for common test cases.
